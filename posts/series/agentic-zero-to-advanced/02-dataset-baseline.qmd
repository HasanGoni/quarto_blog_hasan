...
If you open one of these X-rays cold and think *I have no idea what the labels mean*, that is the
correct first reaction. The defects are real, but they are tiny and low-contrast. GDXray Castings
is a **single-class** dataset: every box just says *a defect is here*, not whether it is porosity,
an inclusion, or a crack. A trained radiographer drew tight rectangles around local density
anomalies -- places where the aluminum is not quite uniform.

Frame `C0001_0001` has only **three** labeled defects. Here they are, numbered on the full frame:

![The three ground-truth boxes on C0001_0001, numbered.](images/baseline-gt-overview.png)

At full resolution (768×572) those boxes are 10–31 pixels wide. Zoomed in, each one wraps a faint
smudge that is only a few grayscale levels darker or brighter than the surrounding metal:

![Zoomed crops of the three labeled defects on C0001_0001.](images/baseline-gt-zoom-panels.png)

That is what you are searching for: not the wheel shape, not the bright air gaps between spokes,
not the spoke edges (the classical baseline flags those constantly) -- just a blob of slightly
wrong density, often sitting on top of structural texture that is already hard to read. On a
laptop screen at blog-post resolution, many of these labels look like rectangles on noise. That
is the task, not a failure of your eyes.
...
```{=html}
<details>
<summary><strong>Show <code>detect_candidates()</code></strong></summary>
```
...
predicts. Think of the answer key a teacher already has before grading anyone's exam.

**Bounding box.** A rectangle marking roughly where something is in an image, not its exact
pixel-perfect shape. For example, "the treasure is somewhere in this square on the map" -- not the
exact spot, just a region.

**IoU (Intersection over Union).** How much a predicted box overlaps a real one, as a fraction of
their combined area. Suppose you and a friend each draw a circle on the ground guessing where a
dropped coin landed -- IoU is how much your two circles overlap, divided by the total ground either
circle covers.

**Precision.** Of everything the model flagged, what fraction was actually real. If you wrote ten
parking tickets today, precision is what fraction of those cars were actually illegally parked.

**Recall.** Of everything that was actually real, what fraction the model actually caught. Suppose
twenty cars were illegally parked today -- recall is what fraction of them you actually ticketed.

**Classical computer vision.** Image analysis via fixed, hand-written rules (thresholds, filters,
morphology) rather than a model that learned patterns from labeled examples. Think of reading a
thermometer versus asking a doctor who's examined a thousand patients -- one follows a rule you
could write down on paper, the other learned from experience.
